namespace :categories_retrieval do
  ...
  task :process_page => :environment do#testing
    require File.join(File.dirname(__FILE__), "categories_parser.rb")
    #puts CategoriesParser::process_category_page(Category.find_or_create_by_url("http://www.pricemania.sk/katalog/auto-moto/"))
    puts CategoriesParser::process_category_page(Category.find_or_create_by_url("http://www.pricemania.sk/katalog/auto-moto/"))
  end
  ...
  task :process_home_page => :environment do#testing
    require File.join(File.dirname(__FILE__), "categories_parser.rb")
    puts CategoriesParser::process_home_page_for_category_pages
  end
end